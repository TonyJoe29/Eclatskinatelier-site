...
The public RSS feed still tops out at `2026-06-12T16:03:27.000Z`, so `Current Favs` was left unchanged instead of resurfacing older pins as if they were new.

Useful corrections shipped on this run:

- Refreshed `data/pinterest-feed-latest.json` so the public Pinterest snapshot reflects the 2026-07-30 fetch even though the 3-day window returned 0 pins.

Products moved into `Current Favs` on this run:

- None. The 2026-07-27 through 2026-07-30 lookback window contained 0 pins.
...
- None. Homepage checks did not surface a stale CTA, disclosure, canonical, duplicate, responsive, or affiliate-link issue that required an edit.

Products moved into `Current Favs` on this run:

- None. The 2026-07-25 through 2026-07-28 lookback window contained 0 pins.

## 2026-07-03

No recent Pinterest pins were returned by `node tools/fetch-pinterest-feed.mjs --days 3 --limit 30 --write`, so no products were eligible for publication.

The public RSS feed still tops out at `2026-06-12T16:03:27.000Z`, so `Current Favs` was left unchanged rather than resurfacing older pins as if they were new.

Useful corrections shipped on this run:

- Refreshed two stale homepage month labels from June to July in `index.html` while keeping the verified Pinterest-picks date range intact.

Products moved into `Current Favs` on this run:

- None. The 2026-06-30 through 2026-07-03 lookback window contained 0 pins.
...
- Fixed homepage/editorial encoding artifacts affecting the site title, Open Graph title, logo text, and Pinterest-picks date separators in `index.html`.

Products moved into `Current Favs` on this run:

- None. The 2026-06-28 through 2026-07-01 lookback window contained 0 pins.
